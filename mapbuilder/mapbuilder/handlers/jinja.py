...
def _render_polygon(lines, polygons, color):
    for polygon in polygons:
        ...
        if len(polygon.coords) > 2:
            ...
            count = len(polygon.coords)
            for idx, point in enumerate(polygon.coords):
                if idx == count - 1:
                    break

                lines.append(
                    f"LINE:{coord2es((point[0], point[1]))}:{coord2es((polygon.coords[idx + 1][0], polygon.coords[idx + 1][1]))}"
                )
            lines.append("")
